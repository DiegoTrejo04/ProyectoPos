...
public class AbarrotesPos {
        ...
        public static void generarTicketPDF(List<DetalleVenta> carrito, double total, double pago, double cambio) {
            ...
            try {
                ...
                for (DetalleVenta d : carrito) {
                    ...
                    table.addCell(String.format("$%.2f", d.subtotal));
                }
                document.add(table);
                document.add(new Paragraph("------------------------------------------------").setTextAlignment(TextAlignment.CENTER));
    ...
    static class ProductoDAO {
        ...
        public Map<String, Object> obtenerEstadisticasHoy() throws SQLException {
            ...
            String sqlVentas = "SELECT SUM(ABS(m.Cantidad) * p.PrecioVenta) as TotalVenta, COUNT(DISTINCT m.IdMovimiento) as Transacciones " +
                "FROM MovimientosInventario m JOIN Productos p ON m.IdProducto = p.IdProducto " +
                "WHERE m.TipoMovimiento = 'VENTA' AND date(m.FechaMovimiento) = date('now','localtime')";
    ...
    static class VentasPanel extends JPanel {
        ...
        private void procesarCobro() {
            ...
            String pStr = JOptionPane.showInputDialog("Total: $" + tot + "\nPago:");
            if (pStr != null) {
                try {
                    double pay = Double.parseDouble(pStr);
                    if (pay >= tot) {
                        for (DetalleVenta d : carrito)
                            productoDAO.actualizarInventario(d.producto.id, -d.cantidad, "VENTA", AbarrotesPos.sesionActual.username);
                ...
                } catch (Exception e) { JOptionPane.showMessageDialog(this, "Error en pago"); }
            }
        }
